...
// Get data where this user is the receiver
$stmt = $sql->prepare("
    SELECT p.*, u.full_name AS sender_name
    FROM p2p p
    JOIN users u ON p.sender = u.id
    WHERE p.reference = ? AND p.receiver = ?
");
...
<div id="appCapsule">
    <div class="section mt-2 mb-2">
        ...
        <div class="listed-detail text-center mt-3">
            ...
            <h3 class="text-success mt-2">Received Successfully</h3>
        </div>

        <ul class="listview flush transparent simple-listview no-space mt-3">
            <li><strong>Transaction Type</strong><span>P2P Received</span></li>
            ...
            <li><strong>Sender Name</strong><span><?= htmlspecialchars($sender_name); ?></span></li>
        ...
        <div class="mt-4">
            <div class="row g-2 text-center">
                ...
                <div class="col-6 col-md-3">
                    <button onclick="shareReceipt()" class="btn btn-outline-success w-100">📤 Share</button>
                </div>
                <div class="col-6 col-md-3">
                    <button class="btn btn-outline-danger w-100" onclick="checkDisputeStatus('<?= $transaction_ref ?>')">⚠️ Dispute</button>
                </div>
            </div>
        </div>

        <small class="text-muted d-block mt-1">
            <i class="fas fa-info-circle"></i> You may raise a dispute if the sender’s details or intent was misleading.
        </small>
    </div>
</div>
